...
export function PacePanel({metrics,events,planningStart}:{metrics:Metrics;events:LedgerEventRow[];planningStart:string}){
  const today=clampDate(new Date().toISOString().slice(0,10),planningStart,ZERO_DAY);
  const target=targetThrough(metrics,planningStart,today);
  const generated=events.filter(e=>(e.event_type==="INCOME"||e.event_type==="ADJUSTMENT")&&e.event_date>=planningStart&&e.event_date<=today).reduce((s,e)=>s+e.amount,0);
  const delta=generated-target;
  const daysElapsed=Math.max(0,inclusiveDays(planningStart,today));
  const daysTotal=inclusiveDays(planningStart,ZERO_DAY);
  const daysRemaining=Math.max(1,inclusiveDays(today,ZERO_DAY));
  ...
  return <section className="panel pace-panel"><div className="panel-head"><div><span className="eyebrow">DAILY GENERATION ENGINE</span><h2>Live pace to Zero Day</h2></div><span className="count">{planningStart} → {ZERO_DAY}</span></div>
    <div className="pace-kpis"><div><span>Today target</span><b>CHF {money(todayTarget)}</b></div><div><span>Cumulative target</span><b>CHF {money(target)}</b></div><div><span>Generated in live window</span><b>CHF {money(generated)}</b></div><div><span>Advance / delay</span><b className={delta>=0?"good":"warn"}>{delta>=0?"+":""} CHF {money(delta)}</b></div><div><span>Catch-up average</span><b>CHF {money(catchupRate)}/day</b></div><div><span>Days remaining</span><b>{daysRemaining}</b></div></div>
